...
public class ExtentReportManager implements ITestListener {
	...
	public void onStart(ITestContext testContext) {
		...
		sparkRepoter.config().setDocumentTitle("RestAssuredReports");
		sparkRepoter.config().setReportName("Pet Store Reports");
		sparkRepoter.config().setTheme(Theme.STANDARD);
		
		extent = new ExtentReports();
		extent.attachReporter(sparkRepoter);
		extent.setSystemInfo("Application", "Pet Store Reports");
		extent.setSystemInfo("Operating System", System.getProperty("os.name"));
		extent.setSystemInfo("User Name", System.getProperty("user.name"));
		extent.setSystemInfo("Env", "QA");
		extent.setSystemInfo("User", "Hero");		
	}
